...
>(function Messages({ sessionId, therapistName, voiceConfigId }, _ref) {
  ...
  const CONFIG_ID_TO_DISPLAY: Record<string, string> = {
    // Trial defaults
    '793d1f15-4bf9-4beb-a4ab-a62caff84e70': 'Male Voice', // Zander
    '3a451da2-a50a-42c2-83fa-13c79f027643': 'Female Voice', // Luna
    // Legacy/defaults from StartCall
    '0ea8bb7d-ef50-4174-ae64-be7a621db425': 'Male Voice',
    '8a80af40-ec14-4da0-afeb-d11008491410': 'Energetic Voice', // Energetic
    'a608626e-23e0-4070-8e24-dc880d34000b': 'The Pirate', // Captain Jack
    // Additional voice configurations
    'ea959dd5-8316-4373-b30e-7d10f1a75631': 'Brit', // Brit
    '7538b2c8-4527-4e56-919f-19523d640a8e': 'Julian', // Julian
    'b4fe69c2-720c-44bb-ae45-eccfcc7ebcd6': 'Aven', // Aven (Male Protagonist)
    '22161928-44b4-4514-b464-e9d8b0636afa': 'Nia', // Nia
    '959e5574-4f1a-4f33-a295-84265c438ab3': 'Zora', // Zora
    '0aae3b3b-b14f-444d-b728-66fb6f0e700f': 'Kai', // Kai
    '8346ae7f-32c4-40f6-aa81-20ce7081df13': 'Sass', // Sass
  };
  ...
  useEffect(() => {
    const fetchAgentName = async () => {
      ...
      try {
        ...
        // Prioritize character name even in error case
        if (mappedDisplay) {
          const characterName = getCharacterName(mappedDisplay, mappedDisplay);
          console.log('✅ Messages: Using character name (error fallback):', characterName);
          setAgentName(characterName);
    ...
    const fetchUserDisplayName = async () => {
      try {
        ...
        if (profile?.full_name) {
          // Use first name if full name is available, otherwise fall back to "You"
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (!hasAnyChat) {
        const displayName = therapistName && therapistName !== 'Terapeuta IA' ? therapistName : agentName;
        const greeting = `¡Hola! Soy ${displayName}. ¿Cómo estás hoy?`;
      ...
      >
        <div className="flex flex-col gap-2">
          <div className="text-sm text-muted-foreground">
            {msg.type === "user_message" ? userDisplayName : agentName}
          </div>
  ...
  }, [combinedMessages, agentName, userDisplayName]);
