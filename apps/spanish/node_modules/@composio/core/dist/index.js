...
var AuthConfigCreationToolAccessConfigSchema = z.object({
  toolsForConnectedAccountCreation: z.array(z.string()).optional()
});
var AuthConfigToolAccessConfigSchema = z.object({
  toolsAvailableForExecution: z.array(z.string()).optional(),
  toolsForConnectedAccountCreation: z.array(z.string()).optional()
});
var AuthSchemeEnum = z.enum([
  "OAUTH2",
  "OAUTH1",
  "API_KEY",
  "BASIC",
...
var CreateCustomAuthConfigParamsSchema = z.object({
...
var AuthConfigListParamsSchema = z.object({
  cursor: z.string().optional(),
  isComposioManaged: z.boolean().optional(),
  limit: z.number().optional(),
  toolkit: z.string().optional()
});
var AuthConfigListResponseSchema = z.object({
  items: z.array(AuthConfigRetrieveResponseSchema),
  nextCursor: z.string().nullable(),
  totalPages: z.number()
});
var AuthCustomConfigUpdateParamsSchema = z.object({
...
var AuthDefaultConfigUpdateParamsSchema = z.object({
...
var AuthConfigUpdateParamsSchema = z.discriminatedUnion("type", [
...
var RedirectableAuthSchemeSchema = z2.enum([
  AuthSchemeTypes.OAUTH1,
  AuthSchemeTypes.OAUTH2
]);
var BaseSchemeRaw = z2.object({
  // for posthog, freshdesk, zendesk, clickup and others
  subdomain: z2.string().optional(),
  // for atlassian
  ["your-domain"]: z2.string().optional(),
  // for mixpanel
  region: z2.string().optional(),
  // for shopify
  shop: z2.string().optional(),
  // for snowflake
  account_url: z2.string().optional(),
  // likely pipedrive
  COMPANYDOMAIN: z2.string().optional(),
  // likely zoho
  extension: z2.string().optional(),
  // likely formsite
  form_api_base_url: z2.string().optional(),
  // likely salesforce
  instanceEndpoint: z2.string().optional(),
  // likely active campaign
  api_url: z2.string().optional(),
  // for borneo
  borneo_dashboard_url: z2.string().optional(),
  // for zenrows proxy
  proxy_username: z2.string().optional(),
  proxy_password: z2.string().optional(),
  // for d2l
  domain: z2.string().optional(),
  version: z2.string().optional(),
  // for mailchimp
  dc: z2.string().optional(),
  // for sharepoint
  site_name: z2.string().optional(),
  // for servicenow
  instanceName: z2.string().optional(),
  // for netsuite
  account_id: z2.string().optional(),
  // for custom servers
  your_server: z2.string().optional(),
  // for ragic
  server_location: z2.string().optional(),
  // base_url only
  base_url: z2.string().optional(),
  // for api key
  api_key: z2.string().optional(),
  // for generic api key
  generic_api_key: z2.string().optional(),
  // for bearer token
  bearer_token: z2.string().optional()
}).catchall(z2.unknown());
var Oauth2InitiatingConnectionDataSchema = BaseSchemeRaw.extend({
  status: z2.literal(ConnectionStatuses.INITIALIZING)
}).catchall(z2.unknown());
var Oauth2InitiatedConnectionDataSchema = Oauth2InitiatingConnectionDataSchema.extend({
  status: z2.literal(ConnectionStatuses.INITIATED),
  code_verifier: z2.string().optional(),
  redirectUrl: z2.string(),
  callback_url: z2.string().optional(),
  finalRedirectUri: z2.string().optional(),
  // previously verification_token, will be sent as verification_token to slack
  webhook_signature: z2.string().optional()
}).catchall(z2.unknown());
var Oauth2ActiveConnectionDataSchema = Oauth2InitiatingConnectionDataSchema.extend({
  status: z2.literal(ConnectionStatuses.ACTIVE),
  access_token: z2.string().optional(),
  id_token: z2.string().optional(),
  token_type: z2.string().optional(),
  refresh_token: z2.string().nullish(),
  expires_in: z2.union([z2.string(), z2.number(), z2.null()]).optional(),
  scope: z2.union([z2.string(), z2.array(z2.string()), z2.null()]).optional(),
  // previously verification_token, will be sent as verification_token to slack
  webhook_signature: z2.string().optional(),
  authed_user: z2.object({
    access_token: z2.string().optional(),
...
var Oauth2FailedConnectionDataSchema = Oauth2InitiatingConnectionDataSchema.extend({
  status: z2.literal(ConnectionStatuses.FAILED),
...
var Oauth2ExpiredConnectionDataSchema = Oauth2InitiatingConnectionDataSchema.extend({
  status: z2.literal(ConnectionStatuses.EXPIRED),
  expired_at: z2.string().optional()
}).catchall(z2.unknown());
var Oauth2InactiveConnectionDataSchema = Oauth2InitiatingConnectionDataSchema.extend({
  status: z2.literal(ConnectionStatuses.INACTIVE)
}).catchall(z2.unknown());
var Oauth2ConnectionDataSchema = z2.discriminatedUnion("status", [
...
var Oauth1InitiatedConnectionDataSchema = Oauth1InitiatingConnectionDataSchema.extend({
  status: z2.literal(ConnectionStatuses.INITIATED),
  oauth_token: z2.string().optional(),
  authUri: z2.string().optional(),
  oauth_token_secret: z2.string().optional(),
  redirectUrl: z2.string().optional(),
  callbackUrl: z2.string().optional()
}).catchall(z2.unknown());
var Oauth1ActiveConnectionDataSchema = Oauth1InitiatingConnectionDataSchema.extend({
  status: z2.literal(ConnectionStatuses.ACTIVE),
  oauth_token: z2.string().optional(),
  consumer_key: z2.string().optional(),
  redirectUrl: z2.string().optional(),
  callback_url: z2.string().optional()
}).catchall(z2.unknown());
...
var Oauth1ConnectionDataSchema = z2.discriminatedUnion("status", [
...
var CustomOauth1ConnectionDataSchema = Oauth1ActiveConnectionDataSchema.omit({
  ...
  oauth_token_secret: z2.string(),
  oauth_token: z2.string()
});
var BillcomAuthInitiatingSchema = BaseSchemeRaw.extend({
  status: z2.literal(ConnectionStatuses.INITIALIZING)
}).catchall(z2.unknown());
var BillcomAuthConnectionDataSchema = z2.discriminatedUnion("status", [
...
var BasicConnectionDataSchema = z2.discriminatedUnion("status", [
...
var ApiKeyConnectionDataSchema = z2.discriminatedUnion("status", [
...
var BearerTokenConnectionDataSchema = z2.discriminatedUnion("status", [
...
var GoogleServiceAccountInitiatingSchema = BaseSchemeRaw.extend({
  status: z2.literal(ConnectionStatuses.ACTIVE),
  credentials_json: z2.string()
}).catchall(z2.unknown());
var GoogleServiceAccountConnectionDataSchema = z2.discriminatedUnion("status", [
...
var NoAuthConnectionDataSchema = z2.discriminatedUnion("status", [
...
var CalcomAuthConnectionDataSchema = z2.discriminatedUnion("status", [
...
var BasicWithJwtConnectionDataSchema = z2.discriminatedUnion("status", [
...
var ConnectionDataSchema = z2.discriminatedUnion("authScheme", [
  z2.object({
    authScheme: z2.literal(AuthSchemeTypes.OAUTH1),
    /**
  ...
  z2.object({
    authScheme: z2.literal(AuthSchemeTypes.OAUTH2),
    /**
     * the main connection data discriminated by auth scheme
     */
  ...
  z2.object({
    authScheme: z2.literal(AuthSchemeTypes.API_KEY),
    /**
     * the main connection data discriminated by auth scheme
     */
  ...
  z2.object({
    authScheme: z2.literal(AuthSchemeTypes.BASIC),
    /**
     * the main connection data discriminated by auth scheme
     */
  ...
  z2.object({
    authScheme: z2.literal(AuthSchemeTypes.BEARER_TOKEN),
    /**
     * the main connection data discriminated by auth scheme
     */
  ...
  z2.object({
    authScheme: z2.literal(AuthSchemeTypes.GOOGLE_SERVICE_ACCOUNT),
    /**
     * the main connection data discriminated by auth scheme
     */
  ...
  z2.object({
    authScheme: z2.literal(AuthSchemeTypes.NO_AUTH),
    /**
     * the main connection data discriminated by auth scheme
     */
  ...
  z2.object({
    authScheme: z2.literal(AuthSchemeTypes.CALCOM_AUTH),
    /**
     * the main connection data discriminated by auth scheme
     */
  ...
  // }),
  z2.object({
    authScheme: z2.literal(AuthSchemeTypes.BILLCOM_AUTH),
    /**
     * the main connection data discriminated by auth scheme
     */
  ...
  z2.object({
    authScheme: z2.literal(AuthSchemeTypes.BASIC_WITH_JWT),
    /**
     * the main connection data discriminated by auth scheme
     */
...
var CustomConnectionDataSchema = z2.discriminatedUnion("authScheme", [
  z2.object({
    authScheme: z2.literal(AuthSchemeTypes.OAUTH2),
    toolkitSlug: z2.string(),
...
// src/types/tool.types.ts
var ToolkitSchema = z3.object({
  slug: z3.string().describe("The slug of the toolkit"),
  name: z3.string().describe("The name of the toolkit"),
  logo: z3.string().describe("The logo of the toolkit").optional()
});
var JSONSchemaType = z3.enum([
  "string",
  "number",
  "integer",
  "boolean",
...
var JSONSchemaPropertySchema = z3.object({
  type: z3.union([JSONSchemaType, z3.array(JSONSchemaType)]).optional(),
  description: z3.string().optional(),
  anyOf: z3.lazy(() => z3.array(JSONSchemaPropertySchema)).optional(),
  oneOf: z3.lazy(() => z3.array(JSONSchemaPropertySchema)).optional(),
  allOf: z3.lazy(() => z3.array(JSONSchemaPropertySchema)).optional(),
  not: z3.lazy(() => JSONSchemaPropertySchema).optional(),
  title: z3.string().optional(),
  default: z3.any().optional(),
  nullable: z3.boolean().optional(),
  properties: z3.lazy(() => z3.record(z3.string(), JSONSchemaPropertySchema)).optional(),
  required: z3.array(z3.string()).optional(),
  file_uploadable: z3.boolean().optional(),
  file_downloadable: z3.boolean().optional(),
  items: z3.lazy(() => z3.union([JSONSchemaPropertySchema, z3.array(JSONSchemaPropertySchema)])).optional(),
  enum: z3.array(z3.any()).optional(),
  const: z3.any().optional(),
  ...
  uniqueItems: z3.boolean().optional(),
  minProperties: z3.number().optional(),
  maxProperties: z3.number().optional(),
  patternProperties: z3.lazy(() => z3.record(z3.string(), JSONSchemaPropertySchema)).optional(),
  additionalProperties: z3.union([z3.boolean(), z3.lazy(() => JSONSchemaPropertySchema)]).optional(),
  examples: z3.array(z3.any()).optional(),
  readOnly: z3.boolean().optional(),
  writeOnly: z3.boolean().optional(),
  if: z3.lazy(() => JSONSchemaPropertySchema).optional(),
  then: z3.lazy(() => JSONSchemaPropertySchema).optional(),
  else: z3.lazy(() => JSONSchemaPropertySchema).optional(),
  $ref: z3.string().optional(),
  definitions: z3.record(
    z3.string(),
    z3.lazy(() => JSONSchemaPropertySchema)
  ).optional(),
  $defs: z3.record(
    z3.string(),
    z3.lazy(() => JSONSchemaPropertySchema)
...
var ParametersSchema = z3.object({
  type: z3.literal("object"),
  anyOf: z3.array(JSONSchemaPropertySchema).optional(),
  oneOf: z3.array(JSONSchemaPropertySchema).optional(),
  allOf: z3.array(JSONSchemaPropertySchema).optional(),
  not: JSONSchemaPropertySchema.optional(),
  properties: z3.record(z3.string(), JSONSchemaPropertySchema),
  required: z3.array(z3.string()).optional(),
  title: z3.string().optional(),
  default: z3.any().optional(),
  nullable: z3.boolean().optional(),
  description: z3.string().optional(),
  additionalProperties: z3.boolean().default(false).optional()
});
var ToolSchema = z3.object({
  slug: z3.string().describe('The slug of the tool. eg. "GOOGLE_SEARCH"'),
  name: z3.string().describe(`The name of the tool. eg. "Google Search"`),
  description: z3.string().optional().describe("The description of the tool"),
  ...
  tags: z3.array(z3.string()).describe("The tags of the tool. eg: Important").default([]).optional(),
  toolkit: ToolkitSchema.describe("The toolkit of the tool").optional(),
  version: z3.string().describe('The version of the tool, e.g. "20250909_00"').optional(),
  availableVersions: z3.array(z3.string()).describe("Available versions of the tool.").default([]).optional(),
  scopes: z3.array(z3.string()).describe('The scopes of the tool. eg: ["task:add"]').optional()
});
var ToolListResponseSchema = z3.object({
  items: z3.array(ToolSchema),
  nextCursor: z3.string().nullable().optional(),
  totalPages: z3.number()
});
var ToolListParamsSchema = z3.object({
  tools: z3.array(z3.string()).optional(),
  toolkits: z3.array(z3.string()).optional(),
  scopes: z3.array(z3.string()).optional(),
  tags: z3.array(z3.string()).optional(),
  limit: z3.number().optional(),
  search: z3.string().optional(),
  authConfigIds: z3.array(z3.string()).optional()
});
var CustomAuthParamsSchema = z3.object({
  baseURL: z3.string().optional(),
  body: z3.record(z3.string(), z3.unknown()).optional(),
  parameters: z3.array(
    z3.object({
      in: z3.enum(["query", "header"]),
      name: z3.string(),
      value: z3.union([z3.string(), z3.number()])
    })
  )
});
var ToolExecuteParamsSchema = z3.object({
  allowTracing: z3.boolean().optional(),
  connectedAccountId: z3.string().optional(),
  customAuthParams: CustomAuthParamsSchema.optional(),
  customConnectionData: CustomConnectionDataSchema.optional(),
  arguments: z3.record(z3.string(), z3.unknown()).optional(),
  userId: z3.string().optional(),
  // we have no auth apps which can be execute
  version: z3.union([z3.literal("latest"), z3.string()]).optional(),
  text: z3.string().optional()
});
var ToolExecuteResponseSchema = z3.object({
  data: z3.record(z3.string(), z3.unknown()),
  error: z3.string().nullable(),
  successful: z3.boolean(),
  logId: z3.string().optional(),
  sessionInfo: z3.unknown().optional()
});
var ToolProxyParamsSchema = z3.object({
  endpoint: z3.string(),
  method: z3.enum(["GET", "POST", "PUT", "DELETE", "PATCH"]),
...
var CreateConnectedAccountLinkOptionsSchema = z4.object({
  /**
...
var CreateConnectedAccountLinkResponseSchema = z4.object({
  redirectUrl: z4.string()
});
...
function transformConnectedAccountResponse(response) {
  const parseState = (state) => {
    try {
      ...
      logger_default.warn("Unsupported auth scheme in connected account state, ignoring state field", {
        error
      });
      return void 0;
    }
  ...
  return transform(response).with(ConnectedAccountRetrieveResponseSchema).using((response2) => ({
    ...
    state: parseState(response2.state),
...
// src/utils/toolkitVersion.ts
var getToolkitVersion = (toolkitSlug, toolkitVersions) => {
...
var ToolkitRetrieveResponseSchema = z5.object({
  ...
  baseUrl: z5.string().optional(),
  getCurrentUserEndpoint: z5.string().optional()
});
...
var MCPAuthOptionsSchema = z8.object({
  isChatAuth: z8.boolean().optional()
});
...
var McpRetrieveResponseSchema = z8.object({
  ...
  authConfigIds: z8.array(z8.string()).optional(),
  mcpUrl: z8.string(),
  commands: z8.object({
    claude: z8.string(),
    cursor: z8.string(),
